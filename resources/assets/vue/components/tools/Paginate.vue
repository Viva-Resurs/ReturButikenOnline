<template lang="pug">
    div.ui.grid.equal.width
        div.column.mobile.only
            div.ui.button.icon.item.dropdown( v-dropdown="" )
                ...
                div.ui.menu
                    ...
                    div.five.ui.basic.buttons
                        button.ui.button( @click="firstPage" )
                            i.angle.double.left.icon
                        button.ui.button( @click="prevPage" )
                            i.angle.left.icon
                        button( v-for="n in totalPages" ":class"="'ui button '+((n==currentPage)?'active':'')" @click="toPage(n)" ) {{n}}
        ...
        div.row.tablet.only(style="position: relative; left: 15px")
            div.item( @click="firstPage" )
                i.angle.double.left.icon
            div.item( @click="prevPage" )
                i.angle.left.icon
            div.item
                i {{ translate('page') }} {{currentPage}} ... {{totalPages}}
            div.item( @click="nextPage" )
                i.angle.right.icon
            div.item( @click="lastPage" )
                i.angle.double.right.icon

            select.selection.dropdown.item#limit(
                v-show="total>limitOptions[0]"
                v-dropdown="" v-model="limit" )
                option.item( v-for="option in limitOptions" ":value"="option") {{option}}
