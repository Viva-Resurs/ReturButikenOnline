        ...
        div.ui.right.vertical.inverted.sidebar.labeled.menu
            div( v-if="user" @click="menuToggle()" )
                ...
                div.item( v-if="$root.isAdmin()" )
                    ...
                    div.menu
                        router-link.item( to="/sections" exact ) {{ translate('nav.list') }}
                div.item( v-if="$root.isAdmin(2)" )
                    i.icon.users
                    div.header {{ translate('users') }}
                    div.menu
                        router-link.item( to="/users" exact ) {{ translate('nav.list') }}
                div.item
                    i.icon.cubes
                    div.header {{ translate('article.header') }}
                    div.menu
                        router-link.item( to="/articles" exact ) {{ translate('nav.list') }}
                        router-link.item( to="/articles/create" exact ) {{ translate('article.add') }}
                div.item( v-if="$root.isAdmin()" )
                    i.icon.tags
                    div.header {{ translate('categories') }}
                    div.menu
                        router-link.item( to="/categories" exact ) {{ translate('nav.list') }}
                div.item
                    div.ui.label.grey {{ user.name }}
                    div.header
                        | {{ translate('nav.logged_in_as') }}
                    div.vertical.menu
                        router-link.item( to="/users/profile" exact ) {{ translate('user.profile') }}
                            i.icon.user
